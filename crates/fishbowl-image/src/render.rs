...
mod tests {
    ...
    fn the_agents_working_directory_is_a_link_into_the_sandbox_rather_than_a_mount() {
        ...
        assert!(
            entrypoint.contains(r#"ln -sfn "${WORK_DIR}" "${FISHBOWL_WORK_ALIAS}""#),
    ...
    fn only_the_gateway_account_can_execute_the_binary_that_carries_a_capability() {
        ...
        assert!(dockerfile.contains("setcap cap_net_admin+ep /usr/local/bin/fishbowl-gateway"));
        assert!(dockerfile.contains("chmod 0050 /usr/local/bin/fishbowl-gateway"));
        assert!(
            dockerfile.contains("chown root:gateway /usr/local/bin/fishbowl-gateway"),
            "{dockerfile}"
        );
    }
    ...
    fn the_first_run_questions_are_answered_for_the_directory_the_agent_opens_in() {
        ...
        assert!(rendered.statusline.contains("printf 'Fishbowl"));
        assert!(
    ...
    fn what_the_agent_holds_is_out_of_the_samples_reach() {
        ...
        assert!(
            dockerfile.contains("visudo -cs -f /etc/sudoers.d/fishbowl"),
            "a rule that does not parse would leave the session unable to detonate at all"
        );
        assert!(dockerfile.contains("chmod 0440 /etc/sudoers.d/fishbowl"));
    }
    ...
    fn the_courier_is_built_from_the_same_tree_and_installed_for_the_researcher() {
        let dockerfile = rendered().dockerfile;
        assert!(
            dockerfile.contains("-p fishbowl-gateway -p fishbowl-courier"),
        ...
        assert!(
            dockerfile.contains("/src/target/release/fishbowl-courier"),
            "{dockerfile}"
        );
        assert!(
            !dockerfile.contains("setcap cap_net_admin+ep /usr/local/bin/fishbowl-courier"),

...
use fishbowl_creds::{Credentials, Lent};
use jiff::Timestamp;
use tokio::{
    net::UnixStream,
...
const BRIEFING_RULE: &str = ".windsurf/rules/fishbowl.md";
...
#[derive(Debug, Parser)]
#[command(about = "Runs an agent on a credential lent by the fishbowl host")]
struct Courier {
    /// Agent to run.
    #[arg(long)]
    agent: Agent,
...
async fn run(courier: &Courier) -> Result<ExitStatus> {
    ...
    fishbowl_creds::remove(&courier.credentials)
        .await
    ...
    match (courier.agent, fetch(&courier.socket).await?) {
        (Agent::Claude, Lent::Bearer(bearer)) => {
            ...
            let credentials = Credentials {
                env: fishbowl_creds::bearer_environment(&bearer.token),
        ...
        (Agent::Devin, Lent::Document(contents)) => {
            fishbowl_creds::install(&courier.credentials, contents.as_bytes())
                .await
                .with_context(|| format!("writing {}", courier.credentials.display()))
        }
        (agent, lent) => {
...
fn spawn(courier: &Courier) -> Result<Child> {
    let mut command = match courier.agent {
        Agent::Claude => {
            ...
            command.envs(fishbowl_creds::launch_environment(&courier.credentials));
            command
        }
        Agent::Devin => {

...
//!
//! Release is the reaper's job. [`Lease::acquire`] spawns `fishbowl reap <id>` as a
        ...
        let hold = loop {
            match lock_file(&path)? {
                ...
                None if held_by_living(&path) => {
                    bail!("session {id} is in use: another fishbowl command has it open")
                }
...
/// What `fishbowl reap <session>` is: wait for the owner's death on standard

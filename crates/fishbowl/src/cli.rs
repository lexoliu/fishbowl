...
pub const IMAGE_REPOSITORY: &str = "ghcr.io/lexoliu/fishbowl";
...
pub const LEGACY_IMAGE_REPOSITORY: &str = "localhost/fishbowl";
...
mod tests {
    ...
    fn no_resume_flag_asks_for_a_new_session() {
        assert_eq!(resume_of(&["fishbowl", "shell"]), None);
    }

    #[test]
    fn a_bare_resume_flag_asks_to_be_offered_a_choice() {
        assert_eq!(
            resume_of(&["fishbowl", "shell", "--resume"]),
            Some(Resume::Pick),
    ...
    fn a_named_resume_flag_asks_for_that_session() {
        assert_eq!(
            resume_of(&["fishbowl", "shell", "--resume", "c0ffee"]),

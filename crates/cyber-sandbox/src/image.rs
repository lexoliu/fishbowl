...
use cyber_sandbox_image::{BuildContext, DEFAULT_BASE_IMAGE, DEFAULT_PROFILE};
...
///
/// # Errors
/// Fails when an explicit `--workspace` is not a cyber-sandbox checkout, when the
...
pub async fn ensure(host: &Host, source: &Source, arch: Arch) -> Result<()> {
    ...
    let Source::Staged(staged) = source else {
        ...
        return host.runtime().image_pull(tag, arch).await.with_context(|| {
            format!(
                "pulling {tag}. If this build of cyber-sandbox is not a released version, \
...
/// The cyber-sandbox checkout rooted at `directory`, when there is one.
///
...
mod tests {
    ...
    fn the_published_tag_names_the_version_and_the_architecture() {
        ...
        assert_eq!(
            reference.as_str(),
            format!(
                "ghcr.io/lexoliu/cyber-sandbox:arm64-{}",
                env!("CARGO_PKG_VERSION")
            )
        );
    }
